#!/usr/bin/env python3
"""
sip_call_ivr_test.py — IVR traversal test using two SIP endpoints

EP1 dials an IVR, watches the live transcript stream for the menu prompt,
sends DTMF "1" (sales), and waits for the IVR to route the call to EP2.
EP2 auto-answers, optionally plays a WAV, and the remainder of the test
follows the same flow as sip_call_p2p_test.py.

Usage:
  python3 sip_call_ivr_test.py [options]
...
Required:
  ...
  --ivr         IVR number/URI for EP1 to dial (default: ivr@sip-server)

Optional:
  --ivr-keyword  Transcript keyword that triggers DTMF   (default: sales)
  --ivr-digit    DTMF digit to send when keyword heard   (default: 1)
  --ivr-wait     Max seconds to wait for IVR menu prompt (default: 60)
...
def step_dial_ivr(ep1_url, ivr_target, sip_server):
    if "@" not in ivr_target:
        ivr_target = f"{ivr_target}@{sip_server}"
    log_info(f"EP1 dialling IVR {ivr_target}...")
    ok, data = api("POST", f"{ep1_url}/api/call", label="dial IVR",
                   json={"target": ivr_target})
    if not ok:
        record("3_dial_ivr", False, "EP1 dial IVR")
        return None
    call_id = data.get("callId") or data.get("call_id")
    log_ok(f"IVR call initiated (callId: {call_id})")
    record("3_dial_ivr", True, "EP1 dial IVR")
    return call_id


def _extract_digit(text, keyword):
    """
    Try to parse the DTMF digit for a keyword from phrases like:
      "press 1 for sales"  /  "for sales press 1"  /  "sales, press 1"
    Returns the digit string, or None if no match.
    """
    kw = re.escape(keyword)
    patterns = [
        rf'press\s+(\d)\s+for\s+\w*{kw}\w*',   # press 1 for sales
        rf'for\s+\w*{kw}\w*\s*,?\s*press\s+(\d)',  # for sales, press 1
        rf'\w*{kw}\w*\s*,?\s*press\s+(\d)',      # sales, press 1
    ]
    for pat in patterns:
...
def step_traverse_ivr(ep1_url, call_id, keyword, fallback_digit, timeout):
    """Watch EP1's live transcript for the IVR menu keyword, then send DTMF.

    The digit is extracted dynamically from the transcript (e.g. 'press 1 for
    sales').  If the pattern isn't found, falls back to fallback_digit.
    """
    ws_url = ep1_url.replace("http://", "ws://").replace("https://", "wss://") + "/transcript"

    log_info(f"Watching EP1 transcript at {ws_url} for keyword '{keyword}' (timeout: {timeout}s)...")

    detected      = threading.Event()
    resolved_digit = [fallback_digit]  # mutable so the closure can write it
    ...
    def on_message(ws_conn, msg):
        try:
            ...
            if data.get("type") == "transcript":
                text = data.get("text", "")
                log_info(f"IVR transcript: {text}")
                if keyword.lower() in text.lower():
                    found = _extract_digit(text, keyword)
                    if found:
                        log_ok(f"Parsed digit '{found}' for '{keyword}' from transcript")
                        resolved_digit[0] = found
                    else:
                        log_warn(f"Could not parse digit from transcript — using fallback '{fallback_digit}'")
                    detected.set()
                    ws_conn.close()
        except Exception:
            pass
    ...
    def on_error(ws_conn, err):
        log_warn(f"Transcript WS error: {err}")
    ...
    if not detected.wait(timeout=timeout):
        ws_app.close()
        log_fail(f"IVR keyword '{keyword}' not detected within {timeout}s")
        return record("3b_ivr_dtmf", False, f"IVR DTMF for '{keyword}'")

    digit = resolved_digit[0]
    label = f"IVR DTMF {digit} for '{keyword}'"

    # Let the IVR finish speaking before sending the tone
    time.sleep(0.5)

    ok, _ = api("POST", f"{ep1_url}/api/dtmf", label=f"DTMF {digit}",
                json={"digit": digit})
    if not ok:
        return record("3b_ivr_dtmf", False, label)

    log_ok(f"DTMF '{digit}' sent — IVR routing to {keyword}")
    return record("3b_ivr_dtmf", True, label)


def step_ep2_answer(ep2_url, timeout=30):
    """Poll EP2 until an incoming call arrives, then answer it."""
    log_info(f"Waiting for EP2 to receive the routed call (timeout: {timeout}s)...")
    start = time.time()
    ep2_call_id = None
    ...
    log_info("Confirming call is connected on EP2...")
    for _ in range(10):
        time.sleep(1)
        _, data = api("GET", f"{ep2_url}/api/status")
...
def step_wait_ivr_connected(ep1_url, timeout=15):
    """Poll EP1 until the IVR leg shows as connected (IVR answered)."""
    log_info(f"Waiting for IVR to answer EP1's call (timeout: {timeout}s)...")
    for _ in range(timeout):
        time.sleep(1)
        _, data = api("GET", f"{ep1_url}/api/status")
        active = (data.get("state") or {}).get("activeCall") or data.get("activeCall") or {}
        if active.get("status") in ("connected", "confirmed", "active"):
            log_ok("IVR answered — call connected on EP1")
            return True
    log_warn("Could not confirm IVR answered within timeout — continuing anyway")
    return True  # non-fatal: recording attempt will surface any real error


def step_start_recording(ep1_url, call_id, result_key="3a_record"):
    log_info("Starting recording on EP1...")
    ok, data = api("POST", f"{ep1_url}/api/record/start", label="start recording",
                   json={"callId": call_id})
    if not ok:
        return record(result_key, False, "EP1 start recording")
    log_ok(f"Recording started — {data.get('filename', '')}")
    return record(result_key, True, "EP1 start recording")
...
def step_hangup(ep1_url, call_id):
    ...
    ok, _ = api("POST", f"{ep1_url}/api/hangup", label="hangup",
                json={"callId": call_id})
    if not ok:
        return record("7_hangup", False, "EP1 hang up call")
    log_ok("Call ended")
    return record("7_hangup", True, "EP1 hang up call")
...
def parse_args():
    ...
    p.add_argument("--ivr",           default=None,
                   help="IVR number/URI for EP1 to dial (default: ivr@sip-server)")
    p.add_argument("--ivr-keyword",   default="sales",
                   help="Transcript word that triggers DTMF (default: sales)")
    p.add_argument("--ivr-digit",     default="1",
                   help="DTMF digit to send when keyword is heard (default: 1)")
    p.add_argument("--ivr-wait",      type=int, default=60,
                   help="Max seconds to wait for IVR menu prompt (default: 60)")
    ...
    p.add_argument("--answer-wait",   type=int, default=30,
                   help="Max seconds to wait for EP2 to receive the routed call (default: 30)")
...
def main():
    ...
    ivr_target = args.ivr or f"ivr@{args.sip_server}"

    ts      = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = os.path.join(args.out_dir, ts)
    os.makedirs(out_dir, exist_ok=True)

    ws_url = args.ep1_url.replace("http://", "ws://").replace("https://", "wss://") + "/transcript"

    print(f"\n{BOLD}{'─' * 56}")
    print("  SIP IVR Traversal Test")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'─' * 56}")
    print(f"  EP1      : {args.ep1_url}  ({args.ep1_user}@{args.sip_server})")
    print(f"  EP2      : {args.ep2_url}  ({args.ep2_user}@{args.sip_server})")
    print(f"  IVR      : {ivr_target}")
    print(f"  Keyword  : '{args.ivr_keyword}'  →  DTMF {args.ivr_digit}")
    print(f"  Transcript WS: {ws_url}")
    if args.wav:
    ...
    # ── Step 3: EP1 dials IVR ─────────────────────────────────────────────────
    log_step(3, f"EP1 dials IVR ({ivr_target})")
    call_id = step_dial_ivr(args.ep1_url, ivr_target, args.sip_server)
    if not call_id:
        log_fail("Cannot continue without an active call")
        print_summary(out_dir)
        sys.exit(1)

    # ── Step 3a: Start recording as soon as IVR answers ───────────────────────
    log_step("3a", "Wait for IVR to answer, then start recording")
    step_wait_ivr_connected(args.ep1_url)
    if args.no_record:
        log_warn("Skipped (--no-record)")
        RESULTS["3a_record"] = {"passed": None, "label": "EP1 start recording"}
    else:
        step_start_recording(args.ep1_url, call_id)

    # ── Step 3b: Traverse IVR ─────────────────────────────────────────────────
    log_step("3b", f"Traverse IVR — listen for '{args.ivr_keyword}', send DTMF {args.ivr_digit}")
    if not step_traverse_ivr(args.ep1_url, call_id,
                              args.ivr_keyword, args.ivr_digit, args.ivr_wait):  # ivr_digit is fallback
        log_warn("IVR traversal failed — attempting EP1 hangup")
        step_hangup(args.ep1_url, call_id)
        print_summary(out_dir)
        sys.exit(1)

    # ── Step 4: EP2 answers (routed by IVR) ───────────────────────────────────
    log_step(4, "EP2 answers IVR-routed call")
    ...
    # ── Step 7: EP1 hangs up ──────────────────────────────────────────────────
    log_step(7, "EP1 ends call")
    step_hangup(args.ep1_url, call_id)
    time.sleep(2)

    # ── Step 8: Download files from EP1 ───────────────────────────────────────
    log_step(8, "Download WAV + capture files from EP1")
    ...
    # ── Step 9: Transcribe ────────────────────────────────────────────────────
    log_step(9, "Transcribe EP1 recording")
    tc_file = None
